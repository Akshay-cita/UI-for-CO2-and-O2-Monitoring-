...
class Threadclass_1(QtCore.QThread):
    ...
    def run(self):  # continuosly running
        while (1):
            a=input("Setting CO2 and O2 data from manual or sensor(m/s)\n")
            if a=='m':
                val_co2= terminal_input.Terminal_co2value()
                val_o2 = terminal_input.Terminal_o2value()
                
                self.terminal_signal.emit(str(val_co2))
                self.terminal_signal_1.emit(str(val_o2))
