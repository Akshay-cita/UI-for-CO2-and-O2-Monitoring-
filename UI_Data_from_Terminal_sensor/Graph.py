...
class Threadclass_3(QtCore.QThread):
    ...
    def run(self):  # continuosly running
        while (1):
            c=input("Setting Graph data from manual or sensor(m/s)\n")
            
            if c=='m':
                terminal_val_1=terminal_input.Terminal_co2value()
                terminal_val_2=terminal_input.Terminal_o2value()
                terminal_val_3=terminal_input.Terminal_humidityvalue()
                terminal_val_4=terminal_input.Terminal_temperaturevalue()
                terminal_val = [terminal_val_1,terminal_val_2,terminal_val_3,terminal_val_4]
                self.signal_1.emit(list(terminal_val))
                time.sleep(2)
            elif c=='s':
                time.sleep(3)
