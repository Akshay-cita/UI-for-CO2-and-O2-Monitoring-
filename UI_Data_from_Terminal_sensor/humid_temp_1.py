...
class Threadclass_2(QtCore.QThread):
    ...
    def run(self):  # continuosly running
        while (1):
            time.sleep(3)
            b=input("Setting Humidity and Temperature data from manual or sensor(m/s)\n")
            
            if b=='m':
                hum=terminal_input.Terminal_humidityvalue()
                temp=terminal_input.Terminal_temperaturevalue()
                self.terminal_signal.emit(str(hum))
                self.terminal_signal_1.emit(str(temp))
            elif b=='s':
                val=test_1.Humidity()
                time.sleep(3)
